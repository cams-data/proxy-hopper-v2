...
The `tag` label on `proxy_hopper_requests_total` and `proxy_hopper_responses_total` comes from the `X-Proxy-Hopper-Tag` request header. Set it to identify which endpoints are consuming IPs or triggering quarantine events:
