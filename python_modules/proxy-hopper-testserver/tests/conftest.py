...
# ---------------------------------------------------------------------------
# Backend factory registry — returns IPPoolStore wrapping each backend
# ---------------------------------------------------------------------------
...
async def backend(backend_name) -> AsyncIterator[IPPoolStore]:
    """A started IPPoolStore of each registered backend type."""
    store, _ = _BACKEND_FACTORIES[backend_name]()
...
@pytest_asyncio.fixture
async def manager(proxies: MockProxyPool, upstream: UpstreamServer, backend: IPPoolStore) -> AsyncIterator[TargetManager]:
