...
3. **`admin-ui` queries a GraphQL field the backend doesn't define.**
   `admin-ui/src/graphql/queries.ts`'s `TARGET_METRICS_QUERY` (used in `TargetsPage.tsx`) queries `targetMetrics(name: ...)`, but `proxy_hopper_webserver/graphql/queries.py`'s `Query` type has no `target_metrics` field/resolver. Decision from the project owner: build real per-target metrics, in two tiers — (a) lightweight in-app counters (total/success/failed requests, rolling avg latency, last-request time) written into the existing `Backend` for Redis-backed deployments without Prometheus (the "homelab" tier — memory-backend deployments still can't carry this cross-process, see item 14), skipped entirely when Prometheus is configured to avoid double-instrumentation cost; (b) when Prometheus *is* configured, the webserver queries it server-side (PromQL over HTTP, same pattern Grafana itself uses) rather than exposing Prometheus to the browser directly, which would bypass the existing auth/RBAC boundary. **Blocked on item 14** landing first (need the embedded admin mode to test the memory-backend tier meaningfully, though the Redis tier doesn't strictly require it).
